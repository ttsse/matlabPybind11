...
geomname = 'cuboid'; % cuboid or diaphragm
...
numPatches = [20 30 40 50 60 70 80];
nLoc = [56]; 
pars.oversamp = 3;
pars.ep = 0.1;%0.5; 
pars.overlap = 0.05;
...
for i = 1:length(nLoc)
    ...
    switch geomname
        case 'diaphragm'
            load('data/geom_prob3_nloc120_np120_ol035_H1_R1.mat');
            fldr = 'src/experiments/diaphragm/'; % Main folder for saving.
            newdata = refineTriMesh(bdata,16);
            newdata.bndSize = bdata.bndSize;
            newdata.vol = bdata.vol;
            bdata = newdata;
        case 'cuboid'
            l = 200; w = 50; h = 5;
    ...
    matfile = ['Cover_' geomname '_Patches_' num2str(pars.numPatches) '_np0_' ...
    ...
    switch geomname
        case 'diaphragm'
            pars.pressure_bottom = -250000;
            pars.pressure_top = -500000;
        case 'cuboid'
            pars.mode = mode;
            switch mode 
                case 'manufactured'
    ...
    switch geomname
        case 'diaphragm'
             exper = ['3D_' geomname '_nLoc' num2str(pars.nLoc) '_Patches' ...
             num2str(pars.numPatches) ...
             '_delta' num2str(pars.overlap) '_q' ...
             num2str(pars.oversamp) '_nu' num2str(pars.nu*100) '_ptop' ...
             num2str(-pars.pressure_top) '_pbot'...
             num2str(-pars.pressure_bottom) ];
        case 'cuboid'
            switch mode 
                case 'manufactured'
                    exper = ['3D_' geomname '_nLoc' num2str(pars.nLoc) '_np0' ...
